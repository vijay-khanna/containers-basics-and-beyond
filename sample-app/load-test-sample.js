/*
# sample-app-load-testing for Load Testing
...
# to Kill nodejs in case hangs / terminal disconnect. 
kill $(lsof -t -i:80)
...
//-- Load Test Scripts Start
app.get('/stats-load-test', (request, response) => {
  ...
  var rspString = {"Message": msg, "Version":ver,"TimeStamp":tsStr,"hostname" :hostname, "remoteIP":remoteIP}
...
app.get('/pi-load-test', (request, response) => {
...
atn2 = ((atan(bigOne.div(5)).times(4)).minus(atan(bigOne.div(239)))).times(4).toPrecision(precision)
//console.log(atn2)
  response.status(200)
  response.send(atn2)
})
